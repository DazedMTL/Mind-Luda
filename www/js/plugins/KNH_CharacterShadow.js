 ...
 * 1.0.2 2018/10/23 隊列非表示時またはパーティからメンバーを外した際にも隊列の影が表示される不具合を修正
 *
 * @param 影画像
...
(function () {
  ...
  Game_Event.prototype.setupShadow = function () {
    ...
    if (page) {
      for (var i = 0; i < page.list.length; i++) {
        if (
          page.list[i].code == 108 &&
          getShadowInfo(page.list[i].parameters[0])
        ) {
  ...
  Game_Player.prototype.refresh = function () {
    ...
    if ($gameParty.leader())
      this.setShadowInfo(getShadowInfo($gameParty.leader().actor().note));
  };
  ...
  Game_Follower.prototype.refresh = function () {
    ...
    if (!this.isVisible()) this.setShadow(false);
  };
