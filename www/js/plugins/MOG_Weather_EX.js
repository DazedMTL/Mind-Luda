...
/*:
 * @plugindesc (v3.3 *) Adiciona novos efeitos de climas.
 * @author Moghunter
 *
 ...
 * - プラグインコマンド / マップのメモタグ
 * ===========================================================================
 * 天候を有効にするには、以下のプラグインコマンドを使用してください。
 *
 * weather ID : TYPE : POWER : SPEED : BLEND_TYPE : FILE_NAME
 *
 *
 ...
 * - オフセットコマンド
 * ===========================================================================
 * weather : ID : TYPE : POWER : SPEED : BLEND_TYPE : FILE_NAME : Z-INDEX : ZOOMFRAMES : ANIMATION_SPEED
 *
 ...
 *
 * -> FRAMES          - パーティクルアニメーションを有効にします。
 *                      画像のフレーム数を設定します。
 *
 * -> ANIMATION_SPEED - アニメーションの速度を設定します。
 *
 * -------------------------------------
 * 例
 *   weather : 0 : 0 : 40 : 1 : Leaf_02B : 1 : 5 : 0.50 : 0.20 : 4 : 20
 ...
 * - 天候の解除
 * ===========================================================================
 * 天候を解除するには、下記のプラグインコマンドを使用してください。
 *
 *      clear_weather : WEATHER_INDEX
 *
 * 全ての天候を解除するには、以下のプラグインコマンドを使用してください。
 *
 *      clear_all_weathers
 *
 * 戦闘の天候を解除するには、以下のプラグインコマンドを使用してください。
 *
 *      battle_weather : false
 *
 * ===========================================================================
 ...
 * ===========================================================================
 * - (version 3.3) -
 *   (BUG FIX) - 6、17、20の天候でリフレッシュしないことの修正
 *
 * - (version 3.2) -
 *   (BUG FIX) - MOG_BattleCameraFrontalプラグインでの
 *               Fog XP Style効果のバグを修正
 *
 * - (version 3.1) -
 *   (NEW) - フォグエフェクトスタイルの Rpg Maker XPを追加。 (4種)
 *   (UPD) - パーティクル量の制限を削除しました。 (ベンチマークテスト)
 *   (BUG FIX) -  Notetags of the Maps に関連したクラッシュの修正
 *   (BUG FIX) -  Battle Camera プラグインとの互換性の修正
 *
 * - (version 3.0) -
 *   (NEW) - 複数の天候を使用可能(最大10)。
 *   (NEW) - 9つの新しい効果を追加。
 *   (NEW) - アニメーションパーティクル(フレーム)。
 *   (NEW) - 天候の可視性の優先順位を設定する可能性。
 *   (NEW) - 天候をカスタマイズするためのNEWしいオプション(スピード、ズーム)
 *   (NEW) - 戦闘中の天候を有効または無効にするコマンドプラグイン。
 *   (NEW) - 地図のNOTETAGSを使って天候を活性化する可能性。
 *   (UPD) - 動くスクリーンの中の粒子の位置の改善。
 *   (BUG FIX) - いくつかのタイプの天候における粒子の初期位置の修正。
 */
...
SpriteWeatherEX.prototype.updateSprites = function (sprite) {
  ...
  sprite.visible = true;
  if (
    Imported.MOG_BattleCameraFrontal &&
    !sprite.refreshMove &&
    $gameParty.inBattle()
  ) {
