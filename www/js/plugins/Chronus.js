...
// ----------------------------------------------------------------------------
// Version
// 1.19.0 2021/07/15 アナログ時計の短針を24時間で1周するよう変更する機能を追加
...
(function () {
  ...
  var parseIntStrict = function (value, errorMessage) {
    ...
    if (isNaN(result))
      throw Error(
        "指定した値[" + value + "]が数値ではありません。" + errorMessage
      );
    return result;
  };
  ...
  var getArgBoolean = function (arg) {
    return (
      (arg || "").toUpperCase() === "ON" || (arg || "").toUpperCase() === "TRUE"
    );
  };
  ...
  Game_Interpreter.prototype.pluginCommandChronus = function (command, args) {
    switch (getCommandName(command)) {
      ...
      case "SET_DAY":
        ...
        var month = getArgNumber(
          args[1],
          1,
          $gameSystem.chronus().getMonthOfYear()
        );
        var day = getArgNumber(
          args[2],
          1,
          $gameSystem.chronus().getDaysOfMonth(month)
        );
      ...
      case "SET_CLOCK_BASE":
        $gameSystem
          .chronus()
          .setClockBaseFile(convertEscapeCharacters(args[0]));
        break;
      case "SET_MINUTE_HAND":
        $gameSystem
          .chronus()
          .setMinuteHandFile(convertEscapeCharacters(args[0]));
        break;
      case "SET_HOUR_HAND":
  ...
  Game_Interpreter.prototype.setSwitchTimer = function (
    args,
    named,
    selfSwitch
  ) {
  ...
  Game_Interpreter.prototype.setSwitchAlarm = function (
    args,
    named,
    selfSwitch
  ) {
  ...
  Game_Interpreter.prototype.getSwitchKey = function (arg, selfSwitch) {
    return selfSwitch
      ? [
          $gameMap.mapId(),
          this.eventId(),
          convertEscapeCharacters(arg).toUpperCase(),
        ]
      : getArgNumber(arg);
  };
  ...
  Game_Interpreter.prototype.command236 = function () {
    ...
    if (!$gameParty.inBattle()) {
      ...
      chronus.setWeatherType(
        Game_Chronus.weatherTypes.indexOf(this._params[0])
      );
  ...
  Game_Map.prototype.isChronicleMetaInfo = function (tagNames, defaultValue) {
    ...
    var tileset = $gamePlayer.isTransferring()
      ? $dataTilesets[$dataMap.tilesetId]
      : this.tileset();
  ...
  Game_Map.prototype.isTimeStopEventRunning = function () {
    if (this.isEventRunning()) {
      if (!this._isTimeStopEventRunning)
        this._isTimeStopEventRunning = this.getTimeStopEventRunning();
    } else {
  ...
  Game_Player.prototype.performTransfer = function () {
    var realTransfer =
      this._newMapId !== $gameMap.mapId() && $gameMap.mapId() > 0;
  ...
  Window_Chronus.prototype.initialize = function () {
    _Window_Chronus_initialize.call(
      this,
      0,
      0,
  ...
  Window_Chronus.prototype.getDefaultHeight = function () {
    return (
      this.standardFontSize() * (this.getDateFormat(2) ? 2 : 1) +
      this.standardPadding() * 2 +
      paramCalendarLineSpacing
    );
  };
  ...
  var _Window_Chronus_standardFontSize =
    Window_Chronus.prototype.standardFontSize;
  Window_Chronus.prototype.standardFontSize = function () {
    return (
      paramCalendarFontSize ||
      _Window_Chronus_standardFontSize.apply(this, arguments)
    );
  };
  ...
  Window_Chronus.prototype.refresh = function () {
    ...
    this.contents.drawText(
      this.getDateFormat(2),
      0,
      height + paramCalendarLineSpacing,
      width,
      height,
      "left"
    );
  ...
  var _Sprite_Chronicle_Clock_initialize =
    Sprite_Chronicle_Clock.prototype.initialize;
  ...
  Sprite_Chronicle_Clock.prototype.createHourHandSprite = function () {
    ...
    handSprite.bitmap = handName
      ? ImageManager.loadPicture(handName)
  ...
  Game_Chronus.prototype.updateBatWeatherLevel = function () {
    ...
    var newLevel = (this._badWeaterLevel || 0).clamp(
      frequency,
      max - frequency
    );
    if (Math.randomInt(10) <= 1) frequency *= 5;
    this._badWeaterLevel = (
      newLevel +
      (Math.randomInt(frequency * 2) - frequency)
    ).clamp(0, max);
    this.updateWeatherType();
    this.updateWeatherPower();
  };
  ...
  Game_Chronus.prototype.updateWeatherType = function () {
    ...
    if (this._badWeaterLevel >= border) {
      type = this.isSnowLand()
        ? 3
        : this._badWeaterLevel >= heavyBorder
        ? 2
        : 1;
    }
  ...
  Game_Chronus.prototype.setScreenWeather = function (swift) {
    $gameScreen.changeWeather(
      this.getWeatherType(),
      this._weatherPower,
      swift ? 0 : this.getEffectDuration()
    );
  };
  ...
  Game_Chronus.prototype.getEffectDuration = function () {
    ...
    return this._timeAutoAdd === 0
      ? 1
      : Math.floor((60 * 5) / (this.getRealAddSpeed() / 10));
  };
  ...
  Game_Chronus.prototype.onBattleEnd = function () {
    ...
    this.addTime(
      this._timeBattleAdd + this._timeTurnAdd * $gameTroop.turnCount()
    );
  };
  ...
  Game_Chronus.prototype.setGameVariable = function () {
    ...
    this.setGameVariableSub(
      "時間帯IDのゲーム変数",
      this.getTimeZone.bind(this)
    );
    this.setGameVariableSub(
      "天候IDのゲーム変数",
      this.getWeatherTypeId.bind(this)
    );
    this.setGameVariableSub(
      "累計時間のゲーム変数",
      this.getTotalTime.bind(this)
    );
    this.setGameVariableSub(
      "累計日数のゲーム変数",
      this.getTotalDay.bind(this)
    );
    this.setGameVariableSub(
      "フォーマット時間の変数",
      this.getFormatTimeFormula.bind(this)
    );
  };
  ...
  Game_Chronus.prototype.getWeekIndex = function () {
    return this.isRealTime()
      ? this._nowDate.getDay()
      : this._dayMeter % this.getDaysOfWeek();
  };

  Game_Chronus.prototype.getYear = function () {
    return this.isRealTime()
      ? this._nowDate.getFullYear()
      : Math.floor(this._dayMeter / this.getDaysOfYear()) + 1;
  };
  ...
  Game_Chronus.prototype.getHour = function () {
    return this.isRealTime()
      ? this._nowDate.getHours()
      : Math.floor(this._timeMeter / 60);
  };

  Game_Chronus.prototype.getMinute = function () {
    return this.isRealTime()
      ? this._nowDate.getMinutes()
      : this._timeMeter % 60;
  };

  Game_Chronus.prototype.getFormatTimeFormula = function () {
    this._disablePadding = true;
    var formula = this.convertDateFormatText(
      getParamString("フォーマット時間の計算式")
    );
  ...
  Game_Chronus.prototype.getDateFormat = function (index) {
    return this.convertDateFormatText(
      getParamString("日時フォーマット" + String(index))
    );
  };
  ...
  Game_Chronus.prototype.convertDateFormatText = function (format) {
    ...
    format = format.replace(
      /MM/gi,
      function () {
        return this.getValuePadding(
          this.getMonth(),
          String(this.getMonthOfYear()).length
        );
      }.bind(this)
    );
    format = format.replace(
  ...
  Game_Chronus.prototype.getTimeZone = function () {
    var timeId = 0;
    settings.timeZone.forEach(
      function (zoneInfo) {
        if (this.isHourInRange(zoneInfo.start, zoneInfo.end))
          timeId = zoneInfo.timeId;
      }.bind(this)
    );
    return timeId;
  };
  ...
  Game_Chronus.prototype.getAnalogueMinute = function () {
    return (
      ...
      (this.isRealTime()
        ? 0
        : (this._frameCount / paramAutoAddInterval) * this._timeAutoAdd)
    );
  };
  ...
  Game_Chronus.prototype.getRotationHourHand = function () {
    ...
    return (
      ((this.getAnalogueHour() % numberOfHour) *
        (360 / numberOfHour) *
        Math.PI) /
      180
    );
  ...
  Game_Chronus.prototype.makeAlarm = function (
    timerName,
    timeNumber,
    switchKey,
    interval
    ...
    var dayMeter =
      this.calcNewDay(year, month, day) - (this._criterionDay || 0);
    var targetTime = dayMeter * 24 * 60 + timeMeter;
    this.makeTimer(
      timerName,
      targetTime - baseTime || 0,
      switchKey,
      true,
      interval
    );
  ...
  Game_Chronus.prototype.makeTimer = function (
    timerName,
    timeout,
    switchKey,
    loop,
  ...
  Game_ChronusTimer.prototype.initialize = function (
    timeout,
    switchKey,
    loop,
    interval
