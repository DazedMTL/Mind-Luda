...
/*:ja
 * @plugindesc (v1.6 *) 消滅エフェクトのアニメーションを拡張します。
 * @author Moghunter
 *
 * @param Default Collapse
 * @text デフォルトの消滅エフェクト
 * @desc 0から10 (-1 = なし / -2 = ランダム)
 * @default 3
 * @type select
 ...
 * - 更新履歴 (version 1.6.1)
 * ===========================================================================
 * - (BUG FIX)     - MOG_EnemyPose プラグインを使用して画像の一部をトリミング
 */
...
Moghunter.collShatterDirection = String(
  Moghunter.parameters["Shatter Direction"] || "Left"
);
Moghunter.collDefaultCollapse = Number(
  Moghunter.parameters["Default Collapse"] || 0
);
...
Game_Battler.prototype.setDefaultCollapse = function () {
  var coltype = Math.min(
    Math.max(Number(Moghunter.collDefaultCollapse), -2),
    10
  );
  if (coltype === -1) {
...
Game_Action.prototype.setCollapseEffect = function (target) {
  if (!this._item || !this._item.object()) {
    return;
  }
...
Sprite_Enemy.prototype.updateCollapse = function () {
  if (this._battler._collpaseData[0] != -1) {
...
Sprite_Battler.prototype.isEnemyP = function () {
  ...
  if (!Imported.MOG_EnemyPoses) {
    return false;
  }
  if (!this._battler.isBPose()) {
    return false;
  }
  return true;
};
...
Sprite_Battler.prototype.setupCollapseEffect = function () {
  if (Imported.MOG_BattlerMotion && this._battler) {
    this._battler.clearBdamage(0);
  }
  if (this.collapseType() < 4 || this.collapseType() > 7) {
    if (this._colpsBitmap) {
      this.bitmap = this._colpsBitmap;
    }
  } else {
...
Sprite_Battler.prototype.updateCollapseShatterPoint = function () {
  ...
  for (var i = 0; i < this._spriteCol.length; i++) {
    ...
    if (this._spriteCol[i].opacity > 0) {
      coldone = false;
    }
  }
  if (coldone) {
    this.removeCollapseEffects();
  }
};
...
Sprite_Battler.prototype.updateCollapseSliceHorz = function () {
  ...
  if (this._spriteCol[0].x >= this._spriteCol[0].sx) {
    ...
    if (this._spriteCol[0].opacity <= 0) {
      coldone = true;
    }
  } else {
...
Sprite_Battler.prototype.updateCollapseEffects = function () {
  ...
  if (this._collEffectWait > 0) {
    ...
    if (this._collEffectWait == 0) {
      this._battler.performCollapseSoundMG();
    }
    return;
  }
  if (this.collapseType() <= 1) {
