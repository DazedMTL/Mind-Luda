...
(() => {
  ...
  const pluginOptions = ["MPP_EncounterEffect_Op1", "MPP_EncounterEffect_Op2"];
  ...
  EncounterEffect.setupCenter = function () {
    ...
    const cy = Math.round(char.screenY() - 24).clamp(bounds.minY, bounds.maxY);
  ...
  EncounterEffect.createRoundPoints = function (startR, baseR, height) {
    ...
    const randomAngle = (n) => {
      return startR + baseAngle * n + (baseAngle * rRate * Math.random()) / 100;
    };
    const randomRadius = () => {
  ...
  EncounterEffect.iteratePointT2 = function (allPoints, callback) {
    const { "Split X": splitX = 12, "Split Y": splitY = 9 } = this._params;
    for (let i = 0; i < splitX; i++) {
  ...
  EncounterEffect.breakFragments = function (start, end, duration) {
    this._fragments
      .slice(start, end)
      .forEach((fragment) => fragment.onBreak(this._color, duration));
  };
  ...
  EncounterEffect.onBattleStart = function () {
    const { "Move Duration": duration = 60, "Scatter Duration": delay = 0 } =
      this._params;
  ...
  Encounter_Fragment.prototype.onBattleStartT2 = function (
    duration,
    direction
  ) {
  ...
  const _Scene_Map_encounterEffectSpeed =
