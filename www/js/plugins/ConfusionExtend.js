...
(function () {
  ...
  // パラメータの取得と整形
  //=============================================================================
  var paramTargetFriendSkill = getParamBoolean([
    "TargetFriendSkill",
    "味方対象スキルの対象",
  ]);
  ...
  Game_BattlerBase.prototype.getRestrictState = function () {
    return this.getRestrictStates().sort(
      this.compareOrderStatePriority.bind(this)
    )[0];
  };

  Game_BattlerBase.prototype.compareOrderStatePriority = function (
    stateA,
    stateB
  ) {
  ...
  Game_Actor.prototype.getUsableSkillIdList = function () {
    ...
    return Game_BattlerBase.prototype.getUsableSkillIdList
      .call(this)
      .concat(skillIds);
  };
  ...
  Game_Action.prototype.getConfusionUsableSkills = function (state) {
    var usableSkillInclude = getMetaValues(state, [
      "使用可能スキル",
      "UsableSkill",
    ]);
    var skillList = [];
    if (usableSkillInclude) {
      ...
      if (usableSkillInclude !== true) {
        skillList = this.filterConfusionUsableSkills(
          skillList,
          usableSkillInclude
        );
      }
  ...
  Game_Action.prototype.filterConfusionUsableSkills = function (
    skillList,
    usableSkillInclude
  ) {
