...
(function () {
  ...
  Sprite_Character.prototype.shiftCharacterPatternX = function (shift) {
    ...
    var fileName = this._characterName.substring(
      this._characterName.lastIndexOf("_")
    );
    if (
      fileName.indexOf("D") != -1 &&
      this._character.isDashing() &&
      !this._prevStopping
    ) {
      shift += 3;
    }
  ...
  Sprite_Character.prototype.characterPatternY = function () {
    ...
    if (fileName.indexOf("Q") != -1 && this._character.direction() % 2 != 0) {
      ...
      if (this._character.direction() % 2 != 0) {
        if (
          this.getAddX(this._prevDir) ===
          this.getAddX(this._character.direction()) * -1
        ) {
          this._prevDir = this.reverseDirection(this._prevDir);
        } else if (
          this.getAddY(this._prevDir) ===
          this.getAddY(this._character.direction()) * -1
        ) {
          this._prevDir = this.reverseDirection(this._prevDir);
        }
  ...
  Game_Character.prototype.findDirectionTo = function (goalX, goalY) {
    ...
    start.y = this.y;
    start.g = 0;
    var longdir = Math.max(
      Math.abs(goalX - start.x),
      Math.abs(goalY - start.y)
    );
    var shortDir = Math.min(
      Math.abs(goalX - start.x),
      Math.abs(goalY - start.y)
    );
    ...
    while (nodeList.length > 0) {
      ...
      for (var j = 0; j < 9; j++) {
        ...
        if (index2 < 0 || g2 < nodeList[index2].g) {
          ...
          neighbor.g = g2;
          var longdir = Math.max(
            Math.abs(goalX - neighbor.x),
            Math.abs(goalY - neighbor.y)
          );
          var shortDir = Math.min(
            Math.abs(goalX - neighbor.x),
            Math.abs(goalY - neighbor.y)
  ...
  Game_Map.prototype.isPassable = function (x, y, d) {
    if (d % 2 === 0) {
      ...
      switch (d) {
        case 1:
          return (
            this.checkPassage(x, y, (1 << (2 / 2 - 1)) & 0x0f) &&
            this.checkPassage(x, y, (1 << (4 / 2 - 1)) & 0x0f)
          );
        case 3:
          return (
            this.checkPassage(x, y, (1 << (2 / 2 - 1)) & 0x0f) &&
            this.checkPassage(x, y, (1 << (6 / 2 - 1)) & 0x0f)
          );
        case 7:
          return (
            this.checkPassage(x, y, (1 << (8 / 2 - 1)) & 0x0f) &&
            this.checkPassage(x, y, (1 << (4 / 2 - 1)) & 0x0f)
          );
        case 9:
          return (
