...
(function (_global) {
  ...
  function _canPass(x, y, d) {
    ...
    if (
      Game_CharacterBase.prototype.isCollidedWithEvents.call(this, x2, y2) ||
      this.isCollidedWithVehicles(x2, y2)
    ) {
      return false;
    }
    return true;
        ...
        if (this.x == _x && this.y < _y && _y - this.y <= _d) {
          if (!st) {
            for (var y = this.y; y < _y - 1; y++) {
              if (!_canPass.call(this, _x, y, dir)) {
                return false;
              }
            }
          }
        ...
        if (this.y == _y && _x < this.x && this.x - _x <= _d) {
          if (!st) {
            for (var x = this.x; x > _x + 1; x--) {
              if (!_canPass.call(this, x, _y, dir)) {
                return false;
              }
            }
          }
  ...
  function _checkAll(_d) {
    ...
    return (
      r ||
      (followers &&
        $gamePlayer.followers()._data.some(function (f) {
